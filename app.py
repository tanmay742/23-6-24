...
def execute_queries(engine, from_year, to_year, from_month, to_month):
    ...
    #     (SELECT COUNT(*) AS total_count FROM backup1 WHERE YEAR(Date) BETWEEN {from_year} AND {to_year} AND MONTH(Date) BETWEEN {from_month} AND {to_month}) AS t
    # ) AS ranked_data
    ...
    query_results_max_rainfall = pd.read_sql(query_max_rainfall, conn)
    ...
    result = {
        ...
        'max_rainfall': query_results_max_rainfall.to_dict(orient='records')[0] if not query_results_max_rainfall.empty else None
...
def get_date_range_data():
    ...
    if 'from_date' in request.form and 'to_date' in request.form:
        ...
        print("Sending data back to the frontend:", percentile_result)
...
#             ) AS sorted_data,
#             (SELECT @rownum := 0) AS r,
#             (SELECT COUNT(*) AS total_count FROM backup1 WHERE DAY(Date) = {day} AND MONTH(Date) = {month} AND YEAR(Date)>={start_year} AND YEAR(Date)<={end_year}) AS t
#         ) AS ranked_data
#         WHERE rownum = FLOOR(0.95 * total_count) + 1;
#     """
